...
@Tag(
        name = "Tasks",
        description =
                "Operation for managing tasks: create, update, deletion, retrieval, "
...
public class TaskController {
    ...
    @Operation(
            ...
            description =
                    "Retrieve task using dynamic search and criteria: "
    ...
    @Operation(
            summary = "Update task",
            description =
                    "Update task`s details such as title, description, status, priority, "
                            + "due date, assignee, or labels")
    public TaskResponseDto update(
            @PathVariable Long id, @RequestBody @Valid TaskUpdateRequestDto request) {
